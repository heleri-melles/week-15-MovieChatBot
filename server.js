...
app.post("/search", (req,res)=>{
    ...
    let movieUrl = `https://api.themoviedb.org/3/search/movie?api_key=c08056179ab0b433543cf2d5a8226f39&query=${userMovieTitle}`;
    let genresUrl = "https://api.themoviedb.org/3/genre/movie/list?api_key=c08056179ab0b433543cf2d5a8226f39&language=en-US";
        ...
        for(let i = 0; i < movieGenreIds.length; i++){
            for(let j = 0; j < movieGenres.length; j++){
                if(movieGenreIds[i] == movieGenres[j].id){
                    movieGenresArry.push(movieGenres[j].name);
                }
            }
        }

        let genresToDisplay = "";
        movieGenresArry.forEach(genre => {
...
app.post('/getmovie', (req, res) => {
	...
	const reqUrl = encodeURI(
		`http://www.omdbapi.com/?t=${movieToSearch}&apikey=d1a5822c`
	);
	http.get(
		reqUrl,
		responseFromAPI => {
